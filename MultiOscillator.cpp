...
float32 OscillatorBank::randomAngle() { return (0.5+(stereoCentre+random()-0.5)*stereoWidth)*Pi; }
...
OscillatorBank::OscillatorBank() : bank(MaxN), cores(9), remainder(MaxN,0),
		sampleRate(44100), invRate(2*Pi/sampleRate) {
...
void OscillatorBank::operator()(const unsigned nSamples,const float32 factor,float32 *buffer) {
	if(!core) return;
